...
function DrDamage:Caster_OnEnable()
	...
	if settings.DisplayType2 then
		...
		if settings.DisplayType2 == "Casts" then
			self.manaBucket = self:RegisterBucketEvent("UNIT_POWER", 2)
		end
	end
...
function DrDamage:UNIT_POWER(unit, power)
...
function DrDamage:CasterCalc( name, rank, tooltip, modify, debug )
	...
	calculation.spellDmgM = (calculation.spellDmgM + calculation.spellDmgM_Add) * (baseSpell.sFactor or 1) --* (baseSpell.HybridDownrank and 1 or calculation.penaltyMod)
	calculation.spellDmgM_dot = (calculation.spellDmgM_dot + calculation.spellDmgM_dot_Add) * (baseSpell.sFactor or 1) --* calculation.penaltyMod
	calculation.spellDmg = calculation.spellDmg * calculation.SPM
